...
int main(int argc,char *argv[])
{
	...
	for(is=1;is<=ns;is++)
	{
		...
		cudaMemset(u0, 0, nnz*nnx*sizeof(float));
		cudaMemset(u1, 0, nnz*nnx*sizeof(float));
		cudaMemset(w0, 0, nnz*nnx*sizeof(float));
		cudaMemset(w1, 0, nnz*nnx*sizeof(float));

		cudaMemset(P, 0, nnz*nnx*sizeof(float));
		cudaMemset(Q, 0, nnz*nnx*sizeof(float));

		cudaMemset(px0, 0, nnz*nnx*sizeof(float));
		cudaMemset(px1, 0, nnz*nnx*sizeof(float));
		cudaMemset(pz0, 0, nnz*nnx*sizeof(float));
		cudaMemset(pz1, 0, nnz*nnx*sizeof(float));
		cudaMemset(qx0, 0, nnz*nnx*sizeof(float));
		cudaMemset(qx1, 0, nnz*nnx*sizeof(float));
		cudaMemset(qz0, 0, nnz*nnx*sizeof(float));
		cudaMemset(qz1, 0, nnz*nnx*sizeof(float));
		...
		for(it=0,t=dt;it<nt;it++,t+=dt)
		{
			...
			update_stress<<<(nnx*nnz+511)/512, 512>>>(nx,nz,nnx,nnz,dt,dx,dz,u1,w1,P,Q,vp,npml,px1,px0,pz1,pz0,qx1,qx0,qz1,qz0,acoffx1,acoffx2,acoffz1,acoffz2,delta,epsilon,theta,fs,ds,zs,is,false);
