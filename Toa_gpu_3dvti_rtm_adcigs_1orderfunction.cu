                            ...
                            float *P, float *Q, float *shot, bool flag)
{
    ...
    if(id<nx*ny)
    {
        if(flag)
        {
            shot[it+nt*ix+nt*nx*iy]=P[npml+nnz*(ix+npml)+nnz*nnx*(iy+npml)]+Q[npml+nnz*(ix+npml)+nnz*nnx*(iy+npml)];
        }else{
...
__global__ void cal_illumination(int nnx, int nny, int nnz,int nx, int ny, int nz, int npml, float *illumination, float *P, float *Q)
{
    ...
    for(iy=0;iy<ny;iy++)
    {
        id=iz+ix*nz+iy*nz*nx;
        if(ix<nx&&iy<ny&&iz<nz)
        illumination[id]+=P[iz+npml+nnz*(ix+npml)+nnx*nnz*(iy+npml)]*P[iz+npml+nnz*(ix+npml)+nnx*nnz*(iy+npml)]+Q[iz+npml+nnz*(ix+npml)+nnx*nnz*(iy+npml)]*Q[iz+npml+nnz*(ix+npml)+nnx*nnz*(iy+npml)];
    }
}
/*************func**************/
        ...
        if(ix<(int)(nx/dcdp)&&iz<nz)
        {
            ...
            if(a>=-1&&a<=1)
            {
                ...
                if(ia<na)
                {
                    ...
                    adcigs[id] += s_P[iz+npml+nnz*(ix*dcdp+npml)+nnz*nnx*(iy*dcdp+npml)]*g_P[iz+npml+nnz*(ix*dcdp+npml)+nnz*nnx*(iy*dcdp+npml)]*cosf(ia*pi/180.0)*cosf(ia*pi/180.0)*cosf(ia*pi/180.0);
                }
            }
        }
...
/**********IS Loop start*******/
   for(is=0;is<ns;is++)
    ...
    for(it=0,t=dt;it<nt;it++,t+=dt)
    {
        ...
        update_vel<<<dimg,dimb>>>(nx,ny,nz,nnx,nny,nnz,npml,dt,dx,dy,dz,s_u0,s_v0,s_w0,s_u1,s_v1,s_w1,s_P,s_Q,coffx1,coffx2,coffy1,coffy2,coffz1,coffz2);
        update_stress<<<dimg,dimb>>>(nx,ny,nz,nnx,nny,nnz,dt,dx,dy,dz,s_u1,s_v1,s_w1,s_P,s_Q,vp,npml,s_px1,s_px0,s_py1,s_py0,s_pz1,s_pz0,s_qx1,s_qx0,s_qy1,s_qy0,s_qz1,s_qz0,acoffx1,acoffx2,acoffy1,acoffy2,acoffz1,acoffz2,deta,epsilu,fsx, dsx, fsy, dsy,zs, is, nsx, true);
    ...
    for(it=nt-1;it>=0;it--)
    {
        ...
        update_vel<<<dimg,dimb>>>(nx,ny,nz,nnx,nny,nnz,npml,dt,dx,dy,dz,g_u0,g_v0,g_w0,g_u1,g_v1,g_w1,g_P,g_Q,coffx1,coffx2,coffy1,coffy2,coffz1,coffz2);
        update_stress<<<dimg,dimb>>>(nx,ny,nz,nnx,nny,nnz,dt,dx,dy,dz,g_u1,g_v1,g_w1,g_P,g_Q,vp,npml,g_px1,g_px0,g_py1,g_py0,g_pz1,g_pz0,g_qx1,g_qx0,g_qy1,g_qy0,g_qz1,g_qz0,acoffx1,acoffx2,acoffy1,acoffy2,acoffz1,acoffz2,deta,epsilu,fsx, dsx, fsy, dsy,zs, is, nsx, true);
